...
function removeDuplicate(str) {
  let newStr = Array.from(new Set(str.toLowerCase().split(""))).join('').trim()
...
function removeDuplicate2(str) {
  let newArr = []
  str.toLowerCase().split('').map(item => newArr.indexOf(item) === -1 ? newArr.push(item) : null)
